...
const ColorSetting = ({ Generate }: ColorSettingsProps) => {
  const colorOptions = [
    '#3B82F6', // blue-500
    '#EF4444', // red-500
    '#10B981', // emerald-500
  ...
  const [selectedColor, setSelectedColor] = useState(() => {
    ...
    if (savedResumeData) {
      try {
        ...
        const resumeColor = parsedData.resume_data?.settings?.color;
        if (resumeColor) {
          console.log('Loaded template color from resumeData:', resumeColor);
          return resumeColor;
        }
      } catch (error) {
    ...
    if (settings) {
      try {
        ...
        const settingsColor = parsedSettings.color;
        if (settingsColor) {
          console.log('Loaded template color from settings:', settingsColor);
  ...
  const handleColorClick = (color: string) => {
    ...
    sessionStorage.setItem("TempColour", color);
  ...
  const updateLocalStorage = (color: string | null) => {
    ...
    if (savedResumeData) {
      ...
      const savedResumeData = getFromLocalStorage('resumeData') || null;
      if (savedResumeData){
      const parsedData = JSON.parse(savedResumeData);
  ...
  const handleClearColor = () => {
    ...
    sessionStorage.removeItem("TempColour");
